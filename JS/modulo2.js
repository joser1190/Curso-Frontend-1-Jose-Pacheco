...
console.log(multiplicar(2, 3));
  ...
  function calculadoraPro() {
    // 1. Diccionario de traducción (puedes ampliarlo)
    ...
    function procesarEntrada(valor) {
        let minusculas = valor.toLowerCase().trim();
        // Si es un número escrito en palabras, lo traduce. Si no, lo convierte a número normal.
        return palabrasANumeros[minusculas] !== undefined ? palabrasANumeros[minusculas] : Number(valor);
    }

    let num1 = procesarEntrada(prompt("Ingresa el primer número (o palabra del 0 al 10)"));
    let num2 = procesarEntrada(prompt("Ingresa el segundo número (o palabra del 0 al 10)"));
    let operacion = prompt("Operación: suma, resta, multiplicacion, division").toLowerCase().trim();

    // 2. Lógica con Switch y validaciones
    switch (operacion) {
        case "suma":
